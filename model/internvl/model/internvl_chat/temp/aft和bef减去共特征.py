...
class Adapter(nn.Module):
    def __init__(self):
        ...
        # self.positions = nn.Parameter(torch.randn(1,256+1, 512))
        self.bef_cls_token = nn.Parameter(torch.randn(1, 1, 512))
        self.aft_cls_token = nn.Parameter(torch.randn(1, 1, 512))
        self.bef_diff = nn.Linear(self.att_dim, self.att_dim)
        self.aft_diff = nn.Linear(self.att_dim, self.att_dim)
        self.cls = nn.Linear(512,512)
        self.common_trans = nn.Sequential(
            nn.Linear(self.att_dim * 2, self.att_dim),
            nn.Dropout(0.1),
            nn.ReLU()
        )
        self.encoder = CrossTransformer(512,8,0.1)
        self.crossatt = CrossTransformer(512,8,0.1)

    # a, b, c, d = all.shape
    # X = all
    # short_mask = get_mask(torch.tensor([b] * c * a, dtype=torch.long), b).cuda()
    # X = self.ntrans(X.permute(0, 2, 1, 3).reshape(a * c, b, d), short_mask)[0]
    # X = X.reshape(a, c, b, d).permute(0, 2, 1, 3).reshape(a * b, c, d)
    # A = self.gnn.build_graph(X)
    # A = A.view(a * b, c, c)
    # graph_mask = get_mask(torch.tensor([b] * a, dtype=torch.long), b).cuda()
    # A = self.etrans(x=A.reshape(a, b, c * c), attn_mask=graph_mask)[0]
    # A = A.reshape(a * b, c, c)
    # A = F.softmax(A, dim=-1)
    # X_o = self.gnn(X, A)
    # X_o += X
    # all = self.satt_pool2(X_o) * X_o
    # all = all.reshape(a, b, c, d)
    # input_befs = all[:, 0, :, :]
    # input_afts = all[:, 1, :, :]
    ...
    def forward(self, x): # bs*num 1024 1024
        res_ = x

        x = self.down_linear(x) #bs*num 256 512
        batch_size_num, p,h = x.shape

        x = x.view(-1,2,p,h)
        # print(x.shape)
        input_1 = x[:,0,:,:].permute(1,0,2)
        input_2 = x[:, 1, :, :].permute(1, 0, 2)
        # print(input_1.shape)
        p,batch_size,h = input_1.shape
        ...
        # positions = self.positions.permute(1,0,2).repeat(1,batch_size,1)
        ...
        input_1_common, att1 = self.crossatt(input_1_common, input_2_common)
        input_2_common, att2 = self.crossatt(input_2_common, input_1_common)
        ...
        output = torch.cat([input_1_diff,input_2_diff],dim = 0)
...
class InternVLChatModel(PreTrainedModel):
    ...
    def extract_feature(self, pixel_values):
        ...
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], -1, vit_embeds.shape[-1])#bs*num 256 4096
