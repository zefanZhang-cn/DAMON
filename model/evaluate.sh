...
if  [ ${DATASET} == "diagnosis" ]; then
  torchrun \
    ...
    --nproc_per_node=${GPUS} \
    --master_port=29520 \
    eval/caption/diagnosis.py --checkpoint ${CHECKPOINT} "${ARGS[@]:2}"
fi


if  [ ${DATASET} == "vqarad" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=15313 \
    eval/caption/vqarad.py --checkpoint ${CHECKPOINT} "${ARGS[@]:2}"
fi


if  [ ${DATASET} == "vqamed2019" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=46891 \
    eval/caption/vqamed.py --checkpoint ${CHECKPOINT} "${ARGS[@]:2}"
fi
...
if  [ ${DATASET} == "pathvqa" ]; then
  torchrun \
    ...
    --nproc_per_node=${GPUS} \
    --master_port=41562 \
    eval/caption/pvqa.py --checkpoint ${CHECKPOINT} "${ARGS[@]:2}"
fi
...
if  [ ${DATASET} == "slake" ]; then
  torchrun \
    ...
    --nproc_per_node=${GPUS} \
    --master_port=54621 \
    eval/caption/slake.py --checkpoint ${CHECKPOINT} "${ARGS[@]:2}"
fi


if  [ ${DATASET} == "mimicdiff_all" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=15124 \
    eval/caption/evaluate_all.py --checkpoint ${CHECKPOINT} "${ARGS[@]:2}"
fi
if  [ ${DATASET} == "mimicdiffv2" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=29231 \
    eval/caption/evaluate_mimicdiff.py --checkpoint ${CHECKPOINT} "${ARGS[@]:2}"
fi


if  [ ${DATASET} == "multiimage" ]; then
  torchrun \
    --nnodes=1 \
    --node_rank=0 \
    --master_addr=127.0.0.1 \
    --nproc_per_node=${GPUS} \
    --master_port=26752 \
    eval/caption/multiimage.py --checkpoint ${CHECKPOINT} "${ARGS[@]:2}"
fi
